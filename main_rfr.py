...
from utils.fair_metric import dp, eo, dp_smooth


def regularized_learning(dataset_loader, x_train, y_train, z_train, x_test, y_test, z_test, \
                        model, logger, penalty_coefficient, \
                        data_fitting_loss, ablation=False, lr=1e-5, num_epochs=10):
    ...
    optimizer = RFR(model.parameters(), base_optimizer, rho=args.rho, lr=lr, weight_decay=0.01)
    ...
    for j in range(num_epochs):
        for i, (x, y, z) in enumerate(dataset_loader):
            ...
            loss_dp0, loss_dp1 = dp_smooth(outputs, z)
            loss_smooth = loss_dp0 + loss_dp1
...
parser.add_argument('--rho', type=float, default=1e-4, help='SAM optimizer ')


parser.add_argument('--ablation', type=str, default='False', help='ablation indicator')
...
for run_time in range(RUNNING_TIME):
    ...
    method = 'align' if args.ablation=='False' else 'align_ablation'
    print('method={method}')
    ...
    if args.shift == "syn":  ## synthetic distribution shift
        log_path = parent_folder + f'{dataset_name}/{args.shift}/{method}/alpha={args.alpha}_beta={args.beta}'
    elif args.shift == "real_iid": ## iid
        log_path = parent_folder + f'{dataset_name}/{args.shift}/{method}'
    else:
        if args.real_shift=='states':    ## distribution shift across state
            log_path = parent_folder + f'{dataset_name}/{args.shift}/{method}/{args.ori_state}-{args.shift_state}'
        else:    ## distribution shift across time
            log_path = parent_folder + f'{dataset_name}/{args.shift}/{method}/{args.ori_time}-{args.shift_time}'

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    fh = logging.FileHandler(log_path + f'/{args.prefix}-rho={args.rho}-hyper={penalty_coefficient}-{run_time}.log', mode='w')
    ...
    regularized_learning(dataset_loader, x_train, y_train, z_train, x_test, y_test, z_test, \
                                model, logger, penalty_coefficient, \
                                data_fitting_loss, ablation=args.ablation, lr=lr, num_epochs=num_epochs)
    if dataset_name == 'crimes':
        loss_train, loss_test, mae_train, mae_test, DP_train, DP_test = evaluate(model, data_fitting_loss, \
                                    x_train, y_train, z_train, x_test, y_test, z_test)
...
if args.model_save=='True':
    checkpoints_path = log_path + f'/{args.prefix}-rho={args.rho}-hyper={penalty_coefficient}.pt'
