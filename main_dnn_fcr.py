...
def fcr_loss(model, inputs, inputs_trans, labels, groups, args):
    # pseudo label
    pseudo_outputs = model(inputs).flatten()
    pseudo_label = torch.softmax(pseudo_outputs.detach(), dim=-1)
    max_probs = torch.maximum(pseudo_label, 1-pseudo_label)
    targets = (pseudo_label>0.5).type_as(labels)
    
    
    mask = max_probs.ge(args.threshold).float()
    logits = model(inputs_trans).flatten()

    loss = torch.tensor(0.0).to(torch.device('cuda:{}'.format(args.gpu)))
    sub_losses = []
    sizes = []
    ...
    for a in range(num_groups):
        for y in range(num_labels):
            ...
            if logits_subgroup.size(0) > 0:
                sub_loss = (F.binary_cross_entropy(logits_subgroup, targets_subgroup, reduction='none') * mask_subgroup).mean()
            else:
                sub_loss = torch.tensor(0.0)
            sub_losses.append(sub_loss)
            sizes.append(sum(mask_subgroup))

    if torch.count_nonzero(torch.tensor(sizes)) == num_groups * num_labels:
        ...
        loss = (F.binary_cross_entropy(logits, targets, reduction='none') * mask).mean()

    return loss

def regularized_learning(dataset_loader, x_train, y_train, z_train, x_test, y_test, z_test, \
                        model, args, logger, penalty_coefficient, \
    ...
    for j in range(num_epochs):
        for i, (x, y, z) in enumerate(dataset_loader):
            ...
            x_trans = x + 0.01 * x[torch.randperm(x.size()[0])]
            loss += penalty_coefficient * fcr_loss(model, x, x_trans, y, z, args) 
...
parser.add_argument('--threshold', type=float, default=0.8, help='Prior Gaussian distribution - varaince amp')
...
for run_time in range(RUNNING_TIME):
    ...
    if args.shift == "syn":  ## synthetic distribution shift
        log_path = parent_folder + f'{dataset_name}/{args.shift}/fcr/alpha={args.alpha}_beta={args.beta}'
    elif args.shift == "real_iid": ## iid
        log_path = parent_folder + f'{dataset_name}/{args.shift}/fcr'
    else:
        if args.real_shift=='states':    ## distribution shift across state
            log_path = parent_folder + f'{dataset_name}/{args.shift}/fcr/{args.ori_state}-{args.shift_state}'
        else:    ## distribution shift across time
            log_path = parent_folder + f'{dataset_name}/{args.shift}/fcr/{args.ori_time}-{args.shift_time}'
